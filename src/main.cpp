...
void runTrackingOnDataset(const std::string& filePath,
    ...
    for (int i = 0; i < 5; i++) {
        for (int j = 0; j < 5; j++) {
            ...
            float confThreshold = 0.1 + (0.1 * i);
            int strikeThreshold = j * 3;


//        float thresholdMultiple = 1.0 - (0.2 * i);
            ...
            fs::path relativeOutputPath = "hung_iou_" + to_string(int(confThreshold * 100)) +
                    "_strike" + to_string(strikeThreshold) +
                    "/data/" + fileName;
            ...
            fs::path resultsPath =
                    "../results/threshold_scan_track_conf_vs_strikes/tracker_results/" + relativeOutputPath.string();
            ...
            trackerManager.trackConfThreshold = confThreshold;
            trackerManager.lossThreshold = strikeThreshold;

            //        trackerManager.thresholdMultiple = thresholdMultiple;
