...
public:
     ...
     */
    inline YOLO11Detector(const std::string &modelPath, const std::string &labelsPath, bool useGPU = false);
     ...
     */
    inline std::vector<Detection> detect(const cv::Mat &image, float confThreshold = 0.4f, float iouThreshold = 0.45f);
     ...
     * @param detections Vector of detections.
     */
    inline void drawBoundingBox(cv::Mat &image, const std::vector<Detection> &detections) const {
     ...
     */
    inline void drawBoundingBoxMask(cv::Mat &image, const std::vector<Detection> &detections, float maskAlpha = 0.4f) const {
...
private:
     ...
     */
    inline cv::Mat preprocess(const cv::Mat &image, float *&blob, std::vector<int64_t> &inputTensorShape);
     ...
     * @return std::vector<Detection> Vector of detections.
     */
    inline std::vector<Detection> postprocess(const cv::Size &originalImageSize, const cv::Size &resizedImageShape,
